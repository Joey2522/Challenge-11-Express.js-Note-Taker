...
//Middleware
app.use(express.urlencoded({ extend: false })); //This middleware is responsible for parsing (transforming data into JSON) incoming data from HTML forms when it is submitted to the server using the application/x-www-form-urlencoded content type. The parsed data is then available in the req.body object for further processing in the application's routes or handlers.
